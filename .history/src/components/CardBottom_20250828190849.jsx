...
function CardBottom() {
  ...
  // O'tgan Hafta harajat
  const lastMonthTotal = 320

  // O'tgan xaftaga qaraganda foizlar
  ...
  return (
    <div className='bg-card-white pt-8 p-10 rounded-[1.25rem]'>
      ...
      <ul className=' w-full flex justify-between gap-2 '>
        {chartData.map((ch) => {
         ...
         return (
          ...
          <span className='w-full mb-8'>{ch.weekdays}</span>
        </li>
         )
        })}
      </ul>
      ...
      <span className="bg-cream w-[460px] h-0.5 mb-8"></span>
      ...
      <span className={`font-medium ${percentChange >= 0 ? "text-green-600" : "text-red-600"}`}>
          {percentChange >= 0 ? `+${percentChange}% from last month` : `${percentChange}% from last month`}
        </span>
        <span className=''>from last month</span>
      </div>
    </div>
  )
}
